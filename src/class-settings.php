...
class Settings {
	...
	public function __construct() {
		...
		add_action( 'network_admin_edit_wp-gitlab-updater', array( $this, 'update_network_options' ) );
	...
	public function create_options_page() {
		...
		<div class="wrap">
			...
			$current_tab = isset( $_GET['tab'] ) ? sanitize_text_field( wp_unslash( $_GET['tab'] ) ) : 'themes';
			if ( isset( $_GET['updated'] ) ) {
				?>
				<div id="message" class="updated notice is-dismissible">
					<p><?php esc_html_e( 'Settings saved.', 'wp-gitlab-updater' ); ?></p></div>
			<?php } ?>
			<h2 class="nav-tab-wrapper">
				...
				foreach ( $tab_items as $key => $name ) {
					...
					echo '<a class="nav-tab ' . esc_attr( $active ) . '" href="?page=wp-gitlab-updater&tab=' . esc_attr( $key ) . '">' . esc_html( $name ) . '</a>';
				}
				?>
			</h2>
			<ul>
				...
				if ( 'themes' === $current_tab ) {
					...
					foreach ( $this->installed_themes as $installed_theme ) {
						...
						?>
						<li><a href="#<?php echo esc_attr( $slug ); ?>"><?php echo esc_attr( $name ); ?></a></li>
						<?php
					}
				} else {
					// Display instructions.
	...
	public function update_network_options() {
		...
		if ( isset( $_POST['option_page'] ) && 'wp-gitlab-updater-themes' === $_POST['option_page'] ) {
			if ( false !== check_admin_referer( 'wp-gitlab-updater-themes-options' ) ) {
				if ( isset( $_POST['wp-gitlab-updater-themes'] ) ) {
					$options = wp_unslash( $_POST['wp-gitlab-updater-themes'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
		...
		if ( isset( $_POST['option_page'] ) && 'wp-gitlab-updater-plugins' === $_POST['option_page'] ) {
			if ( false !== check_admin_referer( 'wp-gitlab-updater-plugins-options' ) ) {
				if ( isset( $_POST['wp-gitlab-updater-plugins'] ) ) {
					$options = wp_unslash( $_POST['wp-gitlab-updater-plugins'] ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	private function section_instructions( $tab ) {
		if ( 'themes' === $tab ) {
			?>
			<p><?php esc_html_e( 'This is a list of all installed themes. To use a private GitLab repo for one or more of them, just enter the details and hit »Save Settings«.', 'wp-gitlab-updater' ); ?></p>
		<?php } else { ?>
			<p><?php esc_html_e( 'This is a list of all installed plugins. To use a private GitLab repo for one or more of them, just enter the details and hit »Save Settings«.', 'wp-gitlab-updater' ); ?></p>
			<?php
		}
	}
	 ...
	 *
	 * @param array $input list of themes.
	 *
	 * @return mixed
	 */
	...
	public function field_cb( $args ) {
		...
		?>
		<input
				id="<?php echo esc_attr( $label_for ); ?>" <?php echo ( isset( $args['readonly'] ) && true === $args['readonly'] ) ? 'readonly' : ''; ?>
				type="text" value="<?php echo esc_attr( $value ); ?>"
				name="wp-gitlab-updater-<?php echo esc_attr( $type ); ?>[<?php echo esc_attr( $settings_array_key ); ?>][<?php echo esc_attr( $value_array_key ); ?>]">
		<?php
